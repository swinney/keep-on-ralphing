...
## Purpose
Make the Ralph runner a first-class **log source** for an external aggregator: emit a single
append-only, turn-correlated `live.log` (runner narration + agent output) alongside the structured
`status.jsonl`, so an operator can watch a loop in realtime by tailing the bind-mounted `.ralph/`. The
kit ships no dashboard and opens no network port — aggregation lives entirely in the operator's own
pipeline (see `docs/recipes/vector-console.md`).
