...
| **Host-side** | `skills/` | The Claude Code plugin: `/ralph-init` (scaffold), `/ralph-status` (report), `/ralph-build-base` (build `ralph-base:v1` from the plugin's bundled `base/`, clone-free), `/ralph-upgrade` (bring an existing project's config up to the current templates). |
...
**`/ralph-init` scaffolds; `/ralph-upgrade` merges changes — they are complementary, not
overlapping.** `/ralph-init` is **no-overwrite** (first-time only): it can seed a new
project but never carries template *changes* into an existing one. Propagating changed
templates into an already-scaffolded project is `/ralph-upgrade`'s job — a confirm-gated
merge that ADDS missing pieces (the `Makefile` `GH_TOKEN`/`check-base` blocks, new
`ralph.conf` keys, `PROMPT.md` clauses, new files) while preserving the operator's edits,
**config channel only** (it defers base-image upgrades to `/ralph-build-base`). To make the
merge precise, `/ralph-init` writes a **tracked** scaffold provenance manifest
(`.ralph-scaffold.json` at the repo root — *not* gitignored `.ralph/`, since it must travel
with the repo to a clone): `{ template_version, files: { <path>: <sha256> } }`. This reuses
the base-image **provenance-stamp** pattern (a content hash) for consumer config; the hash
lets `/ralph-upgrade` classify each file as pristine-since-scaffold (safe to regenerate) vs
customized (insert-only) — a literal three-way merge is impossible since the plugin keeps
only current templates, not the historical version a project scaffolded from. The
`example/` golden reference carries a manifest, and `base/tests/test_conformance.sh` (check
5) fails if its recorded hashes drift from the example files.
...
before implementing** — never trust a test never seen failing), run the **full gate in CI order before every commit**
